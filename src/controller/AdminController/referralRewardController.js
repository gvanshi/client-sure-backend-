  ...
  try {
    ...
    if (referralCount >= 8) {
      ...
      if (!existingReward) {
        ...
        console.log(
          `Admin notified: User ${user.name} completed ${referralCount} referrals`,
        );
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (status === "pending") {
      filteredUsers = users.filter(
        (user) => !user.hasReward || user.latestReward?.status === "pending",
      );
    } else if (status === "rewarded") {
      filteredUsers = users.filter(
        (user) => user.hasReward && user.latestReward?.status === "rewarded",
      );
    }
...
export const awardReferralReward = async (req, res) => {
  try {
    ...
    if (!userId || !rewardAmount) {
      return res
        .status(400)
        .json({ error: "User ID and reward amount are required" });
    }
    ...
    // Send notification email to user using centralized function
    await sendRewardNotification(user, "referral", rewardAmount, customMessage);
