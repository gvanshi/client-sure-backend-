...
const REFERRAL_MILESTONES = {
  referral_8: {
    target: 8,
    reward: 300,
    cycleField: "referral8Cycles",
    resetField: "referral8LastReset",
  },
  referral_15: {
    target: 15,
    reward: 500,
    cycleField: "referral15Cycles",
    resetField: "referral15LastReset",
  },
  referral_25: {
    target: 25,
    reward: 1000,
    cycleField: "referral25Cycles",
    resetField: "referral25LastReset",
  },
};
...
export const checkReferralMilestones = async (referrerId) => {
  try {
    ...
    for (const [milestoneKey, milestone] of Object.entries(
      REFERRAL_MILESTONES,
    )) {
      if (activeReferrals >= milestone.target) {
        await grantMilestoneRewardAndReset(user, milestoneKey, milestone);
        console.log(
          `Milestone ${milestoneKey} cycle completed by user ${user.email}`,
        );
...
const grantMilestoneRewardAndReset = async (user, milestoneType, milestone) => {
  try {
    ...
    const currentCycle =
      (user.milestoneRewards?.[milestone.cycleField] || 0) + 1;

    // Grant temporary tokens (similar to leaderboard prizes)
    // Grant tokens to main balance
    user.tokens = (user.tokens || 0) + milestone.reward;

    // Grant temporary tokens record (optional/legacy)
    ...
    console.log(
      `Cycle ${currentCycle}: Granted ${milestone.reward} tokens to ${user.email} for ${milestoneType}, progress reset`,
    );
  } catch (error) {
...
export const getMilestoneProgress = async (userId) => {
  try {
    const user = await User.findById(userId).select(
      "referralStats milestoneRewards",
    );
    if (!user) return null;
    ...
    for (const [key, milestone] of Object.entries(REFERRAL_MILESTONES)) {
      const cyclesCompleted =
        user.milestoneRewards?.[milestone.cycleField] || 0;
    ...
    const stats = await User.aggregate([
      {
        $group: {
          ...
          totalTokensDistributed: {
            $sum: "$milestoneRewards.totalTokensEarned",
          },
          avgCyclesPerUser: {
            $avg: {
              $add: [
...
export const processReferralReward = async (referredUserId, referrerId) => {
  try {
    ...
    if (referrer) {
      referrer.referralStats.activeReferrals = referrer.referrals.filter(
        (ref) => ref.isActive,
      ).length;
